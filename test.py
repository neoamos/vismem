...
#        print(type(preds[-1]))
#        print(preds[-1].size())
        source = Variable( torch.cat((image,mask_pred), dim=1) , volatile = True ).cuda()
        mask_pred = deep_lab(source)[3].data
        mask_pred = (math.e**logsoftmax(Variable(mask_pred, requires_grad=False)))[:, 1:2, :, :]
        mask_pred = rescale(mask_pred).data

        preds.append(mask_pred.clone())
        mask_pred[mask_pred<0.5]  = 0
        #mask_pred[mask_pred>=0.5] = 1
        mask_pred = 255*(mask_pred-0.5)
    return preds

def masktrack_vismem_pass(images, target):
    rescale = nn.UpsamplingBilinear2d(size = ( images[0].shape[2], images[0].shape[3] ))
    target = torch.from_numpy(target).float().cuda()
    preds = [target]
    h_next = None
...
#        print(preds[-1].size())
        source = Variable( torch.cat((image,mask_pred), dim=1) , volatile = True ).cuda()
        appearance = deep_lab(source)[3]
        mask_pred, h_next = vismem(appearance, target, h_next)
        mask_pred = (math.e**logsoftmax(Variable(mask_pred.data, requires_grad=False)))[:, 1:2, :, :]
        mask_pred = rescale(mask_pred).data
        preds.append(mask_pred.clone())
        mask_pred = 255*(mask_pred-0.5)
